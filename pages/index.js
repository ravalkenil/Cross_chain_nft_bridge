...
import { cipherEth, simpleCrypto, bridgeWallet } from '../engine/config';
import { goeNFT, goeCustody, goeErc20, goerpc } from '../engine/config'
import { bsctNFT, bsctCustody, bsctErc20, bsctrpc } from '../engine/config'
import { mumNFT, mumCustody, mumErc20, mumrpc } from '../engine/config'
import 'sf-font';
...
export default function Home() {
  ...
  const setSource=async()=>{
    ...
    const providera = await web3Modal.connect();
    ...
    console.log("sdfcs",key);
    const wallet1 = await new ethers.Wallet(key, provider);
    console.log(wallet1);
    const contract = new ethers.Contract(sNft, NftABI, wallet1);
  ...
  async function initTransfer() {
    ...
    const hello=await dNFTCont.ownerOf(tokenId)
    console.log("=================>",hello);
    await dNFTCont.ownerOf(tokenId).catch(async (error) => {
      if (error) {
          ...
          const a=await retaindNFT.wait();
          console.log("This is rtainnew",a);
  ...
  return (
    <div>
      ...
      <Container sm="true">
        <img
          src="original.png"
          style={{ maxWidth: "100px", opacity: "90%", marginTop: "20px" }}
        />
        <Grid>
          <Text
            css={{
